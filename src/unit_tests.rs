#[cfg(test)]
mod common {
    use crate::tree_decompositions::nice_tree_decomposition::{
        Bag, NiceTreeDecomposition, NodeData, NodeType,
    };
    use crate::tree_decompositions::tree_structure::{TreeStructure, Vertex};
    use std::collections::HashMap;

    pub fn ntd_test_example() -> NiceTreeDecomposition {
        ...
        nodes_data.insert(
            0,
            NodeData::new(NodeType::Leaf, Bag::from([Vertex::new(0)])),
        );
        nodes_data.insert(
            1,
            NodeData::new(
                NodeType::Introduce,
                Bag::from([Vertex::new(0), Vertex::new(1)]),
            ),
        );
        nodes_data.insert(
            2,
            NodeData::new(NodeType::Forget, Bag::from([Vertex::new(1)])),
        );
        nodes_data.insert(
            3,
            NodeData::new(NodeType::Leaf, Bag::from([Vertex::new(1)])),
        );
        nodes_data.insert(
            4,
            NodeData::new(
                NodeType::Introduce,
                Bag::from([Vertex::new(1), Vertex::new(2)]),
            ),
        );
        nodes_data.insert(
            5,
        ...
        nodes_data.insert(
            6,
            NodeData::new(NodeType::Join, Bag::from([Vertex::new(1)])),
        );
        nodes_data.insert(
            7,
            NodeData::new(
                NodeType::Introduce,
                Bag::from([Vertex::new(1), Vertex::new(3)]),
            ),
        );
        nodes_data.insert(
            8,
            NodeData::new(NodeType::Forget, Bag::from([Vertex::new(3)])),
        );
    ...
    pub fn compare_edge_lists(list1: &Vec<(usize, usize)>, list2: &Vec<(usize, usize)>) -> bool {
        for (u, v) in list1 {
...
pub mod nice_tree_decomposition_tests {
    ...
    use crate::unit_tests::common::ntd_test_example;
    ...
    fn test_stingy_ordering() {
        ...
        let ntd =
            import_ntd("data/nice_tree_decompositions/benchmark_ntds/handmade/ntd_bench_8.ntd")
                .unwrap();
        assert_eq!(
            ntd.stingy_ordering(),
    ...
    fn test_nice_tree_decomposition_basic() {
        ...
        assert_eq!(
            ntd.bag(7),
            Some(&HashSet::from([Vertex::new(1), Vertex::new(3)]))
        );
...
pub mod tree_decomposition_handler_tests {
    ...
    pub fn test_ntd_import() {
        let ntd = ntd_test_example();
        assert_eq!(
            import_ntd("data/nice_tree_decompositions/benchmark_ntds/handmade/ntd_bench_9.ntd")
                .unwrap(),
            ntd
        );
    }
...
pub mod diaz_tests {
    ...
    fn test_dpddata() {
        let to_graph = import_metis("data/metis_graphs/handmade/to_3.graph").unwrap();
        let ntd =
            import_ntd("data/nice_tree_decompositions/benchmark_ntds/handmade/ntd_bench_7.ntd")
                .unwrap();

        let mut dp_data =
            diaz_serna_thilikos::diaz_algorithm::DPData::new(&to_graph, &ntd);
        ...
        // check sorted bags
        assert_eq!(
            *dp_data.sorted_bag(8).unwrap(),
            vec![Vertex::new(0), Vertex::new(2), Vertex::new(3)]
        );
        assert_eq!(
            *dp_data.sorted_bag(16).unwrap(),
            vec![Vertex::new(0), Vertex::new(2), Vertex::new(3)]
        );
        assert_eq!(
            *dp_data.sorted_bag(7).unwrap(),
            vec![Vertex::new(0), Vertex::new(2), Vertex::new(3)]
        );
        assert_eq!(
            *dp_data.sorted_bag(11).unwrap(),
            vec![Vertex::new(0), Vertex::new(3)]
        );
    ...
    fn test_diaz() {
        ...
        let ntd =
            import_ntd("data/nice_tree_decompositions/benchmark_ntds/handmade/ntd_bench_8.ntd")
                .unwrap();
        let i = diaz_serna_thilikos::diaz_algorithm::diaz_serna_thilikos_algorithm(
            &from_graph,
            &ntd,
            &to_graph,
        );
        ...
        let to_graph = import_metis("data/metis_graphs/handmade/to_2.graph").unwrap();
        let ntd =
            import_ntd("data/nice_tree_decompositions/benchmark_ntds/handmade/ntd_bench_6.ntd")
                .unwrap();
        let i = diaz_serna_thilikos::diaz_algorithm::diaz_serna_thilikos_algorithm(
            &from_graph,
            &ntd,
...
pub mod graph_generation_test {
    use crate::file_handler::graph_handler::import_metis;
    use crate::file_handler::tree_decomposition_handler::import_ntd;
    use crate::graph_generation::graph_generation_algorithms::{
        equal_graphs, generate_graphs, generate_possible_edges,
    };
    use crate::unit_tests::common::compare_edge_lists;
    ...
    fn test_generate_possible_edges() {
        ...
        assert!(compare_edge_lists(
            possible_edge_hash.get(&1).unwrap(),
            &vec![(4, 2), (2, 2), (4, 4)]
        ));
        assert!(compare_edge_lists(
            possible_edge_hash.get(&5).unwrap(),
            &vec![(4, 2), (2, 2), (4, 4), (1, 2), (1, 1)]
        ));
        assert!(compare_edge_lists(
            possible_edge_hash.get(&7).unwrap(),
            &vec![(0, 0)]
        ));
        assert!(compare_edge_lists(
            possible_edge_hash.get(&8).unwrap(),
            &vec![(0, 0), (1, 1), (0, 1)]
        ));
        assert!(compare_edge_lists(
            possible_edge_hash.get(&10).unwrap(),
            &vec![(0, 0), (1, 1), (0, 1), (4, 2), (2, 2), (4, 4), (1, 2)]
        ));
        assert!(compare_edge_lists(
            possible_edge_hash.get(&13).unwrap(),
            &vec![
                (0, 0),
                (1, 1),
                (0, 1),
...
pub mod algorithm_comparison_test {
    ...
    use crate::file_handler::tree_decomposition_handler::import_ntd;
    use crate::graph_generation::graph_generation_algorithms::{
        generate_graphs, generate_possible_edges,
    };
    ...
    fn compare_brute_force_with_diaz() {
        ...
        for g in &graphs {
            assert_eq!(
                diaz_serna_thilikos_algorithm(g, &ntd, &second_graph),
                simple_brute_force(g, &second_graph)
            );
        }
    }
}
...
pub mod equivalence_class_algorithm_test {
    ...
    use crate::modified_dp::algorithm::{modified_dp, DPData};
    use crate::tree_decompositions::tree_structure::Vertex;
    use crate::unit_tests::common::compare_edge_lists;
    ...
    fn test_dpddata() {
        ...
        assert_eq!(
            dp_data.index_to_edge(&3),
            dp_data.all_possible_edges().get(3)
        );
        assert_eq!(
            dp_data.index_to_edge(&4),
            dp_data.all_possible_edges().get(4)
        );
        assert_eq!(
            dp_data.index_to_edge(&6),
            dp_data.all_possible_edges().get(6)
        );
        assert_ne!(
            dp_data.index_to_edge(&2),
            dp_data.all_possible_edges().get(3)
        );
        assert_ne!(
            dp_data.index_to_edge(&3),
        ...
        assert_eq!(
            *dp_data.edge_to_index(&(0 as usize, 0 as usize)).unwrap(),
            dp_data
                .all_possible_edges()
                .iter()
                .position(|x| *x == (0, 0))
        ...
        assert_eq!(
            *dp_data.edge_to_index(&(2 as usize, 3 as usize)).unwrap(),
            dp_data
                .all_possible_edges()
                .iter()
                .position(|x| *x == (2, 3) || *x == (3, 2))
                .unwrap()
        );

        // test the possible_edges function
        let pos_edges = dp_data.possible_edges(7).unwrap();
        let edges: Vec<(usize, usize)> = pos_edges
            .iter()
            .map(|x| *dp_data.index_to_edge(x).unwrap())
            .collect();
        assert!(compare_edge_lists(
            &vec![(0, 0), (2, 2), (3, 3), (0, 2), (0, 3), (2, 3)],
            &edges
        ));
        ...
        let imported_reference = import_metis(
            "data/metis_graphs/equivalence_class_algorithm_tests/test_edges_to_graph.graph",
        )
        .unwrap();
    ...
    fn test_equivalence_class_algorithm() {
        ...
        let graphs = generate_graphs(
            ntd.vertex_count() as u64,
            generate_possible_edges(&ntd)
                .get(&ntd.root())
                .unwrap()
                .clone(),
        );
        ...
        for graph in &graphs {
            let pos = graphs_hom
                .iter()
                .position(|(g, _h)| equal_graphs(g, graph))
                .unwrap();
            let diaz = diaz_serna_thilikos_algorithm(graph, &ntd, &to_graph);

            let (_g, h) = graphs_hom.get(pos).unwrap();
