...
pub mod tree_structure {
    ...
    impl TreeStructure {
        ...
        pub fn is_parent_of(&self, p: TreeNode, q: TreeNode) -> bool {
            if let Some(&node) = self.parent(q) {
                p == node
            } else {
                false
            }
        ...
        pub fn add_child(&mut self, p: TreeNode, q: TreeNode) {
            ...
            if max(p, q) >= self.number_of_nodes {
                panic!(
                    "Node index {} out of bounds! Number of nodes is {}",
                    max(p, q),
                    self.number_of_nodes
                );
            }
...
pub mod nice_tree_decomposition {
    ...
    impl NiceTreeDecomposition {
        ...
        pub fn new(
            ...
            let stingy_ordering =
                NiceTreeDecomposition::compute_stingy_ordering(&tree_structure, &nodes_data);
            let unique_vertices = NiceTreeDecomposition::compute_unique_vertices(
                &tree_structure,
                &nodes_data,
                &stingy_ordering,
            );
        ...
        pub fn bag(&self, p: TreeNode) -> Option<&Bag> {
            if let Some(node_data) = self.nodes_data.get(&p) {
                Some(node_data.bag())
            } else {
                None
            }
        ...
        fn compute_unique_vertices(
            tree_structure: &TreeStructure,
            nodes_data: &HashMap<TreeNode, NodeData>,
            stingy_ordering: &Vec<TreeNode>,
        ) -> HashMap<TreeNode, Vertex> {
        ...
        pub fn stingy_ordering(&self) -> Vec<TreeNode> {
            self.stingy_ordering.clone()
        }

        /// This function calculates the stingy ordering when a nice tree decomposition is constructed.
        fn compute_stingy_ordering(
            tree_structure: &TreeStructure,
            nodes_data: &HashMap<TreeNode, NodeData>,
        ) -> Vec<TreeNode> {
            NiceTreeDecomposition::recursive_stingy_ordering(
                tree_structure,
                nodes_data,
                tree_structure.root(),
            )
        ...
        fn recursive_stingy_ordering(
            tree_structure: &TreeStructure,
            nodes_data: &HashMap<TreeNode, NodeData>,
            p: TreeNode,
        ) -> (Vec<TreeNode>, TreeNode) {
            ...
            match node_data.node_type() {
                ...
                NodeType::Introduce | NodeType::Forget => {
                    if let Some(children) = tree_structure.children(p) {
                        if let Some(&q) = children.get(0) {
                            ...
                            let (so, bn) = NiceTreeDecomposition::recursive_stingy_ordering(
                                tree_structure,
                                nodes_data,
                                q,
                            );
                ...
                NodeType::Join => {
                    if let Some(children) = tree_structure.children(p) {
                        ...
                        let (mut so1, bn1) = NiceTreeDecomposition::recursive_stingy_ordering(
                            tree_structure,
                            nodes_data,
                            q1,
                        );
                        let (mut so2, bn2) = NiceTreeDecomposition::recursive_stingy_ordering(
                            tree_structure,
                            nodes_data,
                            q2,
                        );
