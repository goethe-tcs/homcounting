...
pub mod diaz_algorithm {
    ...
    pub fn diaz_serna_thilikos_algorithm(
        ...
        let mut dp_data = DPData::new(to_graph, ntd);
        ...
        for p in stingy_ordering {
            ...
            match ntd.node_type(p) {
                    ...
                    if let Some(&unique_vertex) = ntd.unique_vertex(p) {
                        ...
                        if from_graph.has_edge(unique_vertex, unique_vertex) {
                            ...
                            for image in 0..to_graph.node_count() {
                                ...
                                if to_graph.has_edge(
                                    to_graph.from_index(image),
                                    to_graph.from_index(image),
                                ) {
                                    dp_data.set(p, image as Mapping, 1);
                                } else {
                                    dp_data.set(p, image as Mapping, 0);
                                }
                            }
                        } else {
                ...
                Some(NodeType::Introduce) => {
                    ...
                    let neighbours_of_v: HashSet<Vertex> =
                        HashSet::from_iter(from_graph.neighbors(v));
                    let s_q: Vec<&Vertex> =
                        neighbours_of_v.intersection(ntd.bag(p).unwrap()).collect();
                    ...
                    for f_q in 0..dp_data.max_bag_mappings(q) {
                        for a in 0..to_graph.node_count() {
                            ...
                            let f_prime =
                                dp_data.table_extend(f_q, new_index as Mapping, a as Mapping);
                            ...
                            let condition = {
                                ...
                                for u in &s_q {
                                    ...
                                    let image_of_u = to_graph.from_index(
                                        dp_data.table_apply(f_prime, significance as Mapping)
                                            as usize,
                                    );
                            ...
                            dp_data.set(
                                p,
                                f_prime,
                                *dp_data.get(&q, &f_q).unwrap() * (condition as u64),
                            );
                        }
                    }
                ...
                Some(NodeType::Forget) => {
                    ...
                    for f_prime in 0..dp_data.max_bag_mappings(p) {
                        ...
                        for a in 0..to_graph.node_count() {
                            let f_old = dp_data.table_extend(
                                f_prime,
    ...
    pub fn diaz_serna_thilikos_for_ntd_set(
        ntd: &NiceTreeDecomposition,
        to_graph: &MatrixGraph<(), (), Undirected>,
