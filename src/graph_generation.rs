...
pub mod graph_generation_algorithms {
    ...
    pub fn generate_possible_edges(
        ntd: &NiceTreeDecomposition,
    ) -> HashMap<TreeNode, Vec<(usize, usize)>> {
        ...
        for p in stingy_ordering {
            ...
            match ntd.node_type(p) {
                ...
                Some(NodeType::Join) => {
                    ...
                    if possible_edges.get(q1).unwrap().len()
                        >= possible_edges.get(q2).unwrap().len()
                    {
                        first = q1;
                        second = q2;
                    } else {
    ...
    ) -> Vec<petgraph::matrix_graph::MatrixGraph<(), (), Undirected>> {
        // list of graphsas
    ...
    pub fn equal_graphs(
        a: &petgraph::matrix_graph::MatrixGraph<(), (), Undirected>,
        b: &petgraph::matrix_graph::MatrixGraph<(), (), Undirected>,
    ) -> bool {
        ...
        for u in 0..nodes {
            for v in 0..nodes {
                if a.has_edge(a.from_index(u), a.from_index(v))
                    && !b.has_edge(b.from_index(u), b.from_index(v))
                {
                    return false;
                }
