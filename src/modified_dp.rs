...
pub mod algorithm {
    ...
    impl<'a> DPData<'a> {
        ...
        pub fn new<'b>(
            ...
            let all_possible_edges = generated_possible_edges
                .get(&nice_tree_decomposition.root())
                .unwrap();
        ...
        pub fn get(&self, p: &TreeNode, e: &EdgeList, f: &Mapping) -> Option<&u64> {
            if let Some(mappings) = self.table.get(p) {
                mappings.get(&(*e, *f))
            } else {
                None
            }
        ...
        pub fn max_bag_mappings(&self, node: TreeNode) -> Mapping {
            integer_functions_methods::max_mappings(
                self.nice_tree_decomposition.bag(node).unwrap().len() as Mapping,
        ...
        fn sort_bags(
            nice_tree_decomposition: &NiceTreeDecomposition,
        ) -> HashMap<TreeNode, Vec<Vertex>> {
            let mut sorted_bags = HashMap::new();

            for p in nice_tree_decomposition.stingy_ordering() {
                let mut vertex_vector =
                    Vec::from_iter(nice_tree_decomposition.bag(p).unwrap().iter());
                vertex_vector.sort();
        ...
        pub fn intersection(&self, edge_set_1: EdgeList, edge_set_2: EdgeList) -> EdgeList {
        ...
        pub fn edges_to_graph(&self, edges: EdgeList) -> MatrixGraph<(), (), Undirected> {
            let mut graph: MatrixGraph<(), (), Undirected> =
                petgraph::matrix_graph::MatrixGraph::new_undirected();
        ...
        for p in stingy_ordering {
            match ntd.node_type(p) {
                Some(NodeType::Leaf) => {
                    ...
                    let unique_vertex_loop_index = *dpdata
                        .edge_to_index(&(unique_vertex, unique_vertex))
                        .unwrap();
                ...
                Some(NodeType::Introduce) => {
                    ...
                    if let Some(index) = sorted_q_bag
                        .iter()
                        .position(|&vertex| v.index() < vertex.index())
                    {
                        new_index = index;
                    }
                    ...
                    let possible_edges_of_q_integer =
                        dpdata.edges_to_integer_representation(possible_edges_of_q_integer);

                    // loop over all subsets of possible_edges_until_p

                    for edges in possible_edges_until_p
                        .clone()
                        .iter()
                        .powerset()
                        .collect::<Vec<_>>()
                    {
                        ...
                        let edges_integer =
                            dpdata.edges_to_integer_representation(&edges_without_ref);
                        ...
                        for f_q in 0..dpdata.max_bag_mappings(q) {
                            for a in 0..to_graph.node_count() {
                                ...
                                let f_prime =
                                    dpdata.table_extend(f_q, new_index as Mapping, a as Mapping);
                                ...
                                let condition = {
                                    ...
                                    for u in &s_q {
                                        ...
                                        let significance =
                                            *significance_hash.get(&Vertex::new(*u)).unwrap();

                                        let image_of_u = to_graph.from_index(
                                            dpdata.table_apply(f_prime, significance as Mapping)
                                                as usize,
                                        );
                                ...
                                let old_edges_list =
                                    dpdata.intersection(edges_integer, possible_edges_of_q_integer);
                                dpdata.set(
                                    p,
                                    edges_integer,
                                    f_prime,
                                    *dpdata.get(&q, &old_edges_list, &f_q).unwrap()
                                        * (condition as u64),
                                );
                            }
                        }
                ...
                Some(NodeType::Forget) => {
                    ...
                    let significance_forgotten_vertex = sorted_bag_q
                        .iter()
                        .position(|x| *x == forgotten_vertex)
                        .unwrap();
                        ...
                        .collect::<Vec<_>>()
                    {
                        ...
                        for f_prime in 0..dpdata.max_bag_mappings(p) {
                            ...
                            for a in 0..to_graph.node_count() {
                                let f_old = dpdata.table_extend(
                                    f_prime,
                                    significance_forgotten_vertex as Mapping,
                                    a as Mapping,
                                );
                ...
                Some(NodeType::Join) => {
                    if let Some(children) = ntd.children(p) {
                        ...
                        let possible_edges_of_q1_integer =
                            dpdata.edges_to_integer_representation(possible_edges_of_q1_integer);

                        let possible_edges_of_q2_integer = dpdata.possible_edges(*q2).unwrap();
                        let possible_edges_of_q2_integer =
                            dpdata.edges_to_integer_representation(possible_edges_of_q2_integer);
                            ...
                            .collect::<Vec<_>>()
                        {
                            ...
                            for f in 0..dpdata.max_bag_mappings(p) {
                                let intersection1 = dpdata
                                    .intersection(edges_integer, possible_edges_of_q1_integer);
                                let intersection2 = dpdata
                                    .intersection(edges_integer, possible_edges_of_q2_integer);
